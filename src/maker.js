...
async function maker(url, text, selectedStyle = null) {
   ...
   try {
      ...
      // If `selectedStyle` is provided, validate it against the available styles
      let selected = selectedStyle && types.includes(selectedStyle) ? selectedStyle : types[Math.floor(Math.random() * types.length)];

      let post;
      if (types.length !== 0) {
         post = {
            "radio0[radio]": selected,
            submit,
            token,
            build_server: server,
